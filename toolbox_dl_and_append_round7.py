...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(wudcleanscore.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(wudcleandata.PIN.unique().tolist(),columns=['PIN'])
scorespinlist.to_csv(box_temp+'/WUDpins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/WUDpins_data.csv',index=False)
#3 cases for cleanup:
scorespinlist.loc[scorespinlist.PIN.str.contains(" ")==True] #HCD2545759_ V1 - remove space
scorespinlist.loc[scorespinlist.PIN.str.contains("__")==True] #HCD0894875__V1 (correct PIN also present - just delete)
scorespinlist.loc[scorespinlist.PIN.str.contains("\.")==True] #HCD1139541_V1. (correct PIN also present - just delete)

#HCD2545759_ V1 - remove space
wudcleandata.loc[wudcleandata.PIN.str.contains(" ")==True]
wudcleandata.PIN=wudcleandata.PIN.str.replace(' ','')
wudcleandata.loc[wudcleandata.PIN.str.contains(" ")==True] #not there anymore
wudcleanscore.PIN=wudcleanscore.PIN.str.replace(' ','')

#HCD0894875__V1 and HCD1139541_V1. drop
wudcleandata=wudcleandata.loc[~(wudcleandata.PIN=="HCD0894875__V1")]
wudcleandata=wudcleandata.loc[~(wudcleandata.PIN=="HCD1139541_V1.")]
wudcleanscore=wudcleanscore.loc[~(wudcleanscore.PIN=="HCD0894875__V1")]
wudcleanscore=wudcleanscore.loc[~(wudcleanscore.PIN=="HCD1139541_V1.")]

wudcleandata=subjectsvisits(wudcleandata)
wudcleanscore=subjectsvisits(wudcleanscore)
wudcleanscore.loc[~(wudcleanscore.subject ==wudcleanscore.PIN.str.strip().str[:10])]


len(wudcleandata.PIN.unique())
len(wudcleanscore.PIN.unique())

wudcleandata.to_csv(box_temp+'/WashU_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
wudcleanscore.to_csv(box_temp+'/WashU_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497550762901,box_temp+'/WashU_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497567003988,box_temp+'/WashU_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(wuacleanscore.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(wuacleandata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/WUApins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/WUApins_data.csv',index=False)
#no special cases, so just cleanup
wuacleandata=subjectsvisits(wuacleandata)
wuacleanscore=subjectsvisits(wuacleanscore)
wuacleanscore.loc[~(wuacleanscore.subject ==wuacleanscore.PIN.str.strip().str[:10])]
len(wuacleandata.PIN.unique())
len(wuacleanscore.PIN.unique())


wuacleandata.to_csv(box_temp+'/WashU_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
wuacleanscore.to_csv(box_temp+'/WashU_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497557440921,box_temp+'/WashU_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497575919229,box_temp+'/WashU_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
...
#nothing to update this round so just grab curated
umnddata,umndscores = box2dataframe(fileid=UMND)  # 309 in each now
...
#no pins to drop from removelist
umnddata.loc[umnddata.PIN.isin(removelist)]
umnddata.shape
umndscores.shape

#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(umndscores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(umnddata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/UMNDpins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/UMNDpins_data.csv',index=False)
#no special cases, so just cleanup
umnddata=subjectsvisits(umnddata)
umndscores=subjectsvisits(umndscores)
umndscores.loc[~(umndscores.subject ==umndscores.PIN.str.strip().str[:10])]


umnddata.to_csv(box_temp+'/UMN_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
umndscores.to_csv(box_temp+'/UMN_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497544818446,box_temp+'/UMN_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497577708289,box_temp+'/UMN_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
#remove 'only' from box name'
...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(umnascores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(umnadata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/UMNApins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/UMNApins_data.csv',index=False)
#no wierd pin issues, so just cleanup
umnadata=subjectsvisits(umnadata)
umnascores=subjectsvisits(umnascores)
umnascores.loc[~(umnascores.subject ==umnascores.PIN.str.strip().str[:10])]
len(umnadata.PIN.unique())
len(umnascores.PIN.unique())



umnadata.to_csv(box_temp+'/UMN_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
umnascores.to_csv(box_temp+'/UMN_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497534582474,box_temp+'/UMN_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497573405263,box_temp+'/UMN_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(uclaascores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(uclaadata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/UCLAApins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/UCLAApins_data.csv',index=False)
#one wierdo
#HCA8532174_V1_
uclaadata.loc[uclaadata.PIN=="HCA8532174_V1_",'PIN']="HCA8532174_V1"
uclaadata.loc[uclaadata.PIN=="HCA8532174_V1_"]
uclaascores.loc[uclaascores.PIN=="HCA8532174_V1_",'PIN']="HCA8532174_V1"
uclaascores.loc[uclaascores.PIN=="HCA8532174_V1_"]
len(uclaadata.PIN.unique())
len(uclaascores.PIN.unique())

#cleanup
uclaadata=subjectsvisits(uclaadata)
uclaascores=subjectsvisits(uclaascores)
uclaascores.loc[~(uclaascores.subject ==uclaascores.PIN.str.strip().str[:10])]
len(uclaadata.PIN.unique())
len(uclaascores.PIN.unique())


uclaadata.to_csv(box_temp+'/UCLA_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
uclaascores.to_csv(box_temp+'/UCLA_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497555431651,box_temp+'/UCLA_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497578027088,box_temp+'/UCLA_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(ucladscores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(ucladdata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/UCLADpins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/UCLADpins_data.csv',index=False)
#quite a few 'X1' counterparts to 'V1s' ????
ucladdata=subjectsvisits(ucladdata)
ucladscores=subjectsvisits(ucladscores)
ucladscores.loc[~(ucladscores.subject ==ucladscores.PIN.str.strip().str[:10])]
len(ucladdata.PIN.unique())
len(ucladdata.PIN.unique())
ucladscores.groupby('visit').count()

ucladdata.to_csv(box_temp+'/UCLA_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
ucladscores.to_csv(box_temp+'/UCLA_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497530804032,box_temp+'/UCLA_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497571441619,box_temp+'/UCLA_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
...
#nothing to add this round...but maybe stuff to drop from removelist
...
harvcleandata=harvcleandata.loc[~(harvcleandata.PIN.isin(removelist))]
harvcleanscores=harvcleanscores.loc[~(harvcleanscores.PIN.isin(removelist))]
...
#do another round on subject/visit cleaning
scorespinlist=pd.DataFrame(harvcleanscores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(harvcleandata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/HARVpins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/HARVpins_data.csv',index=False)
#one special case of parent report being lower case pin
harvcleandata.loc[harvcleandata.PIN=='HCD2897891_v1','PIN']='HCD2897891_V1'
harvcleanscores.loc[harvcleanscores.PIN=='HCD2897891_v1','PIN']='HCD2897891_V1'

len(harvcleandata.PIN.unique())
len(harvcleanscores.PIN.unique())

harvcleandata=subjectsvisits(harvcleandata)
harvcleanscores=subjectsvisits(harvcleanscores)
harvcleanscores.loc[~(harvcleanscores.subject ==harvcleanscores.PIN.str.strip().str[:10])]

len(harvcleandata.PIN.unique())
len(harvcleanscores.PIN.unique())


harvcleandata.to_csv(box_temp+'/Harvard_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
harvcleanscores.to_csv(box_temp+'/Harvard_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497530866864,box_temp+'/Harvard_HCD_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497579203898,box_temp+'/Harvard_HCD_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
#remove 'only' and date from box fileneames


#MGH Nothing to update this round just do subject qc and removelist################################
MGH2=82761770877
MGHattn=96148925420
MHGcorr=84799213727
...
mghdata=mghdata.loc[~(mghdata.PIN.isin(removelist))]
mghscores=mghscores.loc[~(mghscores.PIN.isin(removelist))]
len(mghdata.PIN.unique())
len(mghscores.PIN.unique())
...
#check PINs for wierdness
scorespinlist=pd.DataFrame(mghscores.PIN.unique().tolist(),columns=['PIN'])
datapinlist=pd.DataFrame(mghdata.PIN.unique().tolist(),columns=['PIN'])
#check for odd balls
scorespinlist.to_csv(box_temp+'/MGHpins_scores.csv',index=False)
datapinlist.to_csv(box_temp+'/MGHpins_data.csv',index=False)
#one wierd
#HCA9913090_ V1
mghdata.loc[mghdata.PIN=='HCA9913090_ V1','PIN']='HCA9913090_V1'
mghscores.loc[mghscores.PIN=='HCA9913090_ V1','PIN']='HCA9913090_V1'

mghdata=subjectsvisits(mghdata)
mghscores=subjectsvisits(mghscores)
mghscores.loc[~(mghscores.subject ==mghscores.PIN.str.strip().str[:10])]
...
mghdata.to_csv(box_temp+'/MGH_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
mghscores.to_csv(box_temp+'/MGH_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497555470963,box_temp+'/MGH_HCA_Toolbox_Scored_Combined_Round7_'+snapshotdate+'.csv')
box.update_file(497572353229,box_temp+'/MGH_HCA_Toolbox_Raw_Combined_Round7_'+snapshotdate+'.csv')
