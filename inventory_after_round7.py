...
#this is a bad place for a ssaga tester, but whatever.
...
#https://wustl.box.com/s/fhl23lsw3asbpycjseeg73a0opjnddtz
...
len(harvcleandata.PIN.unique()) #550
len(harvcleanscores.PIN.unique()) #550
...
WUDcorr=84801037257
wuddata,wudscores=box2dataframe(fileid=WashUD) #322 in each now
...
len(wuadata.PIN.unique()) #246 in each now
len(wuascores.PIN.unique()) #246 in each now
...
len(umnadata.PIN.unique()) #308 in each now
len(umnascores.PIN.unique()) #308 in each now
...
len(umnddata.PIN.unique()) #271 in each now
len(umndscores.PIN.unique()) #271 in each now
...
len(uclaadata.PIN.unique()) #262
len(uclaascores.PIN.unique()) #262
...
allcurated=subjectsvisits(allcurated)
allcurated.groupby(['site','visit']).count()

alla=pd.concat([M,WA,UMA,UCA],axis=0)
alld=pd.concat([H,WD,UMD,UCD],axis=0)

doubles=pd.read_csv(box_temp+'/DoubleWinners_inRelease.csv')
for i in doubles.HCA_expected_PIN:
    if i in alla.PIN.to_list():
        print(i)
doubles=pd.read_csv(box_temp+'/DoubleWinners_inRelease.csv')
for i in doubles.HCD_expected_PIN:
    if i in alla.PIN.to_list():
        print(i)
...
for i in doubles.HCD_expected_PIN:
    if i in allcurated.PIN.to_list():
        print(i)
...
#raw:
allrawdataHCA=pd.concat([uclaadata,umnadata,wuadata,mghdata],axis=0)
allrawdataHCD=pd.concat([ucladdata,umnddata,wuddata,harvcleandata],axis=0)
len(allrawdataHCA.PIN.unique()) #1046
len(allrawdataHCD.PIN.unique()) #1494

#scores:
allscoresHCA=pd.concat([uclaascores,umnascores,wuascores,mghscores],axis=0,sort=True)
allscoresHCD=pd.concat([ucladscores,umndscores,wudscores,harvcleanscores],axis=0,sort=True)
...
allscoresHCA=allscoresHCA[scorecolumns]
allscoresHCD=allscoresHCD[scorecolumns]
len(allscoresHCA.PIN.unique()) #1046
len(allscoresHCD.PIN.unique()) #1494

######################
#make csv
allrawdataHCA.to_csv(box_temp+'/HCA_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
allrawdataHCD.to_csv(box_temp+'/HCD_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
allscoresHCA.to_csv(box_temp+'/HCA_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
allscoresHCD.to_csv(box_temp+'/HCD_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
##############################
allscoresHCA.loc[allscoresHCA.PIN=='HCD4366971_V1']
